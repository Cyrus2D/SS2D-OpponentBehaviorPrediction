...
class State:
    ...
    def make(self, line, pt):
        ...
        for i in range(len(self.players)):
            self.players[i].features.polar = pos.make_polar(pos.vector(self.kicker.pos, self.players[i].pos))
...
def next_kick_tm(now: State, lines, line_num, pt):
    for l_n, line in enumerate(lines):
        ...
        next = State(line,pt)
        # if now.kicker.unum == next.kicker.unum:
        #     break
        if next.cycle - now.cycle <= 1:
            continue
        if next.side_playing == "r":
            break
